...
7. Read any other `docs/` file relevant to the requested phase.
8. Inspect the repository. Do not rebuild systems that already exist. Do not duplicate services, models, or protocols.
9. Run the currently documented baseline tests, if any exist for the accepted phases.
10. Implement only the requested phase. Do not continue into a later phase.
...
Canonical git line is **`origin/main`**. Last accepted phase is **NPC-01** ([docs/PROGRESS.md](docs/PROGRESS.md)). Four-class combat, progression UI, leftover reset, the live-formula simulator, Hearthworn starter-zone grass, the expanded village stone-road map, C01 walk, NPC contract conflict closure, and `scripts/local-play` belong on `main`. Continue new work on **`origin/main`**. Do not treat stacked `cursor/*` branches as the playable line.

Before editing:

- `git fetch origin main` and work from **`origin/main`**.
- Read [docs/PROGRESS.md](docs/PROGRESS.md), [docs/progression/](docs/progression/), and [docs/npc/](docs/npc/).
- Do not implement later PROG, NPC, or ACCT features until a later phase names them.
