...
func show_hit(world_pos: Vector2, amount: int, against_player: bool) -> void:
	_spawn_number(world_pos, amount, Color(0.95, 0.28, 0.22) if against_player else Color(0.98, 0.86, 0.28))


func show_heal(world_pos: Vector2, amount: int) -> void:
	_spawn_number(world_pos, amount, Color(0.35, 0.92, 0.42))


func _spawn_number(world_pos: Vector2, amount: int, color: Color) -> void:
	if get_tree() == null:
		return
	var label := Label.new()
