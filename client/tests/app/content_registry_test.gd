...
func test_content_hash_is_exposed() -> void:
	...
	assert_str(hash).is_equal("3b1fe2f9b196850d6434503f6c2262867bcd81e6c811c6291cd0510649186084")
	...
	assert_bool(ContentRegistry.has_id("class.mystic")).is_true()
	assert_bool(ContentRegistry.has_id("stat.strength")).is_true()
	assert_bool(ContentRegistry.has_id("curve.vibecode.l10")).is_true()
	assert_bool(ContentRegistry.has_id("ability.warrior.heavy_strike")).is_true()
	...
	assert_bool(ContentRegistry.has_id("route.stationary")).is_true()
	assert_int(ContentRegistry.ids_of_kind("npc_route").size()).is_greater_equal(1)
