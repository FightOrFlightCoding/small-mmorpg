...
func test_platform_npcs_are_content_only() -> void:
	...
	assert_bool(ContentRegistry.has_id("npc.qa_merchant")).is_true()
	assert_bool(ContentRegistry.has_id("npc.qa_herb_bush")).is_true()
	assert_bool(ContentRegistry.has_id("item.wild_herb")).is_true()
	assert_bool(ContentRegistry.has_id("item.qa_green_relic")).is_true()
