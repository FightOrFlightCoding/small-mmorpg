...
func test_payload_is_message_only() -> void:
	...
	var party_body: Dictionary = ZoneChat.party_payload("  hello  ", "p_one")
	assert_str(String(party_body["message"])).is_equal("hello")
	assert_str(String(party_body["partyId"])).is_equal("p_one")
	assert_int(party_body.size()).is_equal(2)
