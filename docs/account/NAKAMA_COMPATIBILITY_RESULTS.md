...
Live suite on this machine: **10/10 passed** (2026-08-19) after `scripts/backend-up.ps1`. No blocker. Email verification **delivery** is implemented in ACCT-02. ACCT-03 makes verification a gameplay join gate for email accounts (`requirePlayableUser`). Device/debug accounts without a profile remain playable.
...
| verify email | Gateway issues an `EMAIL_VERIFICATION` challenge and SendGrid delivers the code to the player inbox (Mailpit only on automated-test Compose). Confirm sets project `ACTIVE` + `verifiedAt`. Unverified email accounts cannot create or enter a character. Do not fake verification. |
...
| logout all | Gateway `POST /v1/auth/logout-all` then Nakama empty-token logout. Nakama’s session cache is a **blacklist**: a token is invalid only when `exp - tokenExpirySec < lastInvalidation` (strict `<`). Tokens issued in the **same Unix second** as logout-all can remain valid. The fake gateway and live callers wait until the next second after issuing the sessions being revoked (live proof used 1500 ms). Account can authenticate again afterward. Godot character select exposes logout-all with password confirmation. |
