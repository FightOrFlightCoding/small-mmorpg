...
Production `class.*` characters overlay `canonicalLevelCurveId` `curve.vibecode.l10` at runtime. Authored `levelCurveId` remains `test.curve.standard` so the generated content hash stays `3b57502b4a197972c970420cd7b2a5a74955311b5840be0b4d184843c24e3320`. `XP_to_next(level) = round_to_tens(100 * level^1.5)` uses `Math.round(value / 10) * 10`. Cap is 10; leftover XP at cap uses `project.xp.cap_overflow_policy` = `lifetime_only` (`currentXp` / `xpIntoLevel` stay 0). Test classes keep Foundation cap 5, `attributePointsPerLevel` / `skillPointsPerLevel`, and content `xpReward`.

Kill XP for production classes is `8 + 2 * enemy_level`; elite tags multiply by 3. The unused `8 + 3 * level` dial is not implemented. `enemy.green_slime` is level 1, so KillXP is 10 and Prompt 18 slime XP is unchanged. Quest slime-problem 20 stays `project.quest.slime_problem.xp`. Stored grant fields are `eventId`, `characterId`, `reasonType`, `reasonId`, `amount`, `createdAt`. Authorized development grants are the existing GM XP command (`reasonType` `admin`); domain tests may use `dev`. Duplicate event ids do not grant twice.
...
The Metronome Law regression is the documented current result `5 / (1.5/1.12) = 3.73 ≤ 3.8`.

## 2026-09-16 — PROG-12 Mystic, Charms, and Curses

Mystic auto-attack, Fateweave, Protective Charm, Blessing, Benediction, Wither, Evil Eye, and Malediction are `runtimeEnabled: true` on the existing generic combat path. No generic module branches on `class.mystic`.

Target relation selects Fateweave polarity: hostile 20 INT spell harm, ally or self 22 SPI heal. R2 raises both polarities 25%. Compassion adds a 20%/4s HoT after an eligible mend. Malice adds a 20%/4s DoT after an eligible harm; that DoT never crits. Protective Charm is 40 SPI absorb, 18 mana, 8s cooldown, 6s duration; R2 +30% absorb; R3 heals 15% of absorb once on break or natural expiry, including refresh/replace. Battle Blessing arms after an eligible mend and is consumed once on the next eligible harm (+25%). Devotion is +8%/+16% healing. Blessing is 25 mana / 20s cooldown, nearby allies +10% damage for 10s; R2 also +8 percentage points of damage reduction. Mending Ward regenerates 2% of target maximum HP per second while this Mystic’s shield remains. Overflow heals the Mystic for 20% of healing dealt to others and cannot recurse. Benediction heals nearby allies for 30% maximum HP and shields 10% maximum HP for 4s at zero mana with a 90s cooldown.

Wither deals 36 INT-scaled damage over eight ticks in 8s, 1.5s cast, 14 mana, never crits. R2 +25% total. R3 makes affected enemies deal 10% less damage. Siphon heals 15% of Fateweave harm. Dark Bargain reduces tagged curse costs 10%/20%. Evil Eye is 15 mana / 12s cooldown and makes the target take 15% more damage for 8s. Festering increases this Mystic’s DoT tick rate 20% without changing total damage. Vampiric Curse heals 10% of this Mystic’s DoT damage and cannot recurse. Contagion transfers Wither at full remaining design duration to one valid nearby enemy on death (radius 80, server-authoritative, idempotent). Malediction applies the current Wither rank to all eligible nearby enemies and they deal 15% less damage while that Wither runs; Wither R3 and Malediction reductions remain separate sources. Malediction is zero mana / 90s cooldown.
...
Six ordinary residential exteriors (`residence_01`–`residence_06`) sit on `zone.starter` as static map content. Production sprites are 512×448 RGBA PNGs with origin (256, 400), binary exterior alpha, and shadow alpha 96. Godot instances `res://scenes/world/buildings/residential/res_house_0N.tscn` under `EntityRegistry` (Y-sorted with avatars at z_index 6). Server-authoritative collision is extra AABBs in `zone.starter.json` (original 6 kept). DoorAreas and EntranceMarkers are future-ready; interiors are not implemented.
...
Stuck “Waiting for the server…” on some NPCs (Cert Quartermaster, Platform Questgiver) is not authored behavior. The server already returns `INTERACTION_RESULT` for those NPCs. A presenter `Timer` was not a reliable leave-loading path in the live World tree: it could be stopped when a result matched and presentation then failed, and tests only invoked the timeout callback directly. `NpcInteractionWindow` now times out from its own `_process` (`PROCESS_MODE_ALWAYS`) so waiting copy cannot stick even if the presenter timer never fires. Present and vendor/inn/cave handlers coerce option/service/stock lists so a JSON `null` cannot throw a typed `Array` assignment (that would abort later signal receivers). A late `INTERACTION_RESULT` can still present after the waiting copy is replaced. Recreate Nakama after this build.
