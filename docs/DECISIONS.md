...
Remote agents cannot write `C:\Users\Eszter\small-mmorpg`. Playable asset and catalog changes land on **`origin/main`**. `scripts/local-play.ps1` / `scripts/local-play.sh` restore Godot `.import` dirt under `client/addons`, `client/assets`, and `client/resources` (so checkout is not blocked), fast-forward `main`, assert client/server `contentHash` match, and recreate Nakama. A `CONTENT_MISMATCH` / "content pack does not match" dialog means the Godot client loaded a new bundle while Nakama still had an old `server/build`. Do not commit GLoot `.import` diffs.
