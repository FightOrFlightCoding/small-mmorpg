...
| `npc` | `npc.elder.json` plus test vendor/inn/herald/cave NPCs, `npc.proof_giver.json`, and development-only `npc.lab_*` | One generic NPC type. Fields: `id`, `displayNameKey`, `zoneId`, `homePosition`, `routeId`, `interactionRange`, `visualId`, `services`. Services: `dialogue`, `quest_offer`, `quest_turn_in`, `vendor`, `inn`, `healer`, `cave_entrance`, `cave_exit`, `respec`. No elder/merchant/innkeeper classes. |
| `npc_route` | `route.stationary.json` | Cosmetic route types `stationary`, `loop`, `ping_pong`, `weighted_route_graph`. NPC-02 authors and validates routes; poses stay at home until a later movement phase ticks them. |
