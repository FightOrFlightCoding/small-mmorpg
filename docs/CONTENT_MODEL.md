...
| `npc` | `npc.elder.json` plus test vendor/inn/herald/cave NPCs, `npc.proof_giver.json`, NPC-07 `npc.platform_*` proof NPCs, and development-only `npc.lab_*` | One generic NPC type. Fields: `id`, `displayNameKey`, `zoneId`, `homePosition`, `routeId`, `interactionRange`, `visualId`, `services`. Services: `dialogue`, `quest_offer`, `quest_turn_in`, `vendor`, `inn`, `healer`, `cave_entrance`, `cave_exit`, `respec`. No elder/merchant/innkeeper classes. A new ordinary NPC is content-only. |
| `npc_route` | `route.stationary.json`, plus NPC-07 `route.platform_short_loop` and `route.platform_weighted` | Cosmetic route types `stationary`, `loop`, `ping_pong`, `weighted_route_graph`. The match ticks authored routes. Plans are not persisted. |
...
| `vendor` | `vendor.test_general.json`, `vendor.platform_kiosk.json`, development-only `vendor.lab_general.json` | Static stock, server prices, sell multiplier. Infinite quantity. `currencyId` must be `gold`. |
