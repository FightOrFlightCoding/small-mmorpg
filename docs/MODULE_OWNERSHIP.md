...
| `InventoryService` | C | Inventory mirror; pickup/destroy/split/move/overflow-recover/ground-drop/ground-pickup intents | GLoot inventory clone + overflow list | GLoot 3.0.2 | `NetworkService` | no | PICKUP / PICKUP_GROUND_ITEM / DROP_ITEM / DESTROY_ITEM / SPLIT_STACK / MOVE_ITEM / RECOVER_OVERFLOW_ITEM | no grants |
...
| `AbilityService` | C | Ability/hotbar/cast/cooldown mirror; use/cancel/unlock/hotbar intents | In-memory unlocked ids, hotbar (4 production / 8 test), resources, cooldowns, active cast, effects | none | `NetworkService`, `ContentRegistry`, `AttackIntent` | no | USE_ABILITY / CANCEL_CAST / ASSIGN_HOTBAR / UNLOCK_ABILITY | no |
...
| `VendorService` | C | MerchantWindow; buy intents; bag preferred-slot purchases | Last approved vendor NPC, session, selected stock | none | `NetworkService` | no | VENDOR_BUY / VENDOR_SELL | no prices |
...
| `combat.ts` / `combat_pipeline.ts` / `targeting.ts` / `enemy_ai.ts` / `enemy_ability.ts` / `threat.ts` / `boss.ts` / `canonical_combat.ts` / `combat_events.ts` / `combat_rng.ts` | S | Shared combat resolver, targeting, AI profiles, threat, boss phases, death, respawn, generic mechanic handlers, event bus, server random | enemy/player combat fields in match | none | `loot.ts`, `loot_table.ts`, `xp_hooks.ts`, `spawn_controller.ts` | no | COMBAT_EVENT / SET_TARGET / RELEASE_RESPAWN | no |
...
| `ability.ts` | S | Ability use, casts, cooldowns, Foundation unlock, hotbar, independent multi-hit, vault | match casts/cooldowns; progression unlocks/hotbar | none | effects, combat_pipeline, stats, canonical_talents.ts, targeting, movement | serialize via progression | USE_ABILITY / CANCEL_CAST / ASSIGN_HOTBAR / UNLOCK_ABILITY | no |
| `canonical_talents.ts` | S | Derived ability ownership, talent purchase, production 4-slot hotbar | none | none | canonical_progression.ts, stats.ts | no | PURCHASE_TALENT / ASSIGN_HOTBAR via loop | no |
| `effects.ts` | S | Structured effect handlers (damage, heal, resource, modifier, periodic, stun, root, slow, shield, taunt, interrupt, reflect, forced movement) | match effect lists | none | combat_pipeline, stats, threat | no | COMBAT_EVENT | no |
| `stats.ts` | S | Deterministic derived-stat pipeline | none | none | equipment modifiers | no | no | no |
| `progression.ts` / `progression_store.ts` (domain) | S | XP, levels, single/batch free-stat allocation, trainer respec, talent purchase, serialize progression | none | none | stats.ts, canonical_respec.ts, canonical_talents.ts | serialize only | ALLOCATE_ATTRIBUTES / ALLOCATE_ATTRIBUTES_BATCH / TRAINER_RESPEC / SELECT_BRANCH / PURCHASE_TALENT via loop | no |
...
| `loot.ts` / `corpse.ts` / `loot_roll.ts` / `ground_item.ts` | S | Ground loot TTL; corpse containers; Need/Greed rolls; player ground drops and pickup | match loot/corpse/roll/ground-item lists | none | `inventory.ts`, `item_txn.ts`, `combat_rng.ts`, `movement.ts` | no | PICKUP / PICKUP_GROUND_ITEM / DROP_ITEM / corpse opcodes / SUBMIT_LOOT_ROLL | grant in memory |
...
| Godot login vs `auth-gateway/` | Product email register/login/verify/refresh/logout/reset/password-change/email-change/export/delete go through `AccountService` to the gateway. Debug Alice/Bob still use device auth. Secrets stay in the gateway process. Forgotten-email help is copy-only. Support lookup is administrator-only. |
