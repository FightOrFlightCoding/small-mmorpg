...
| `AccountService` | auth-gateway HTTP | Product register/verify/login/refresh/logout/logout-all, in-memory access and refresh tokens, account-status signals, data export, account deletion, error-code mapping. Does not own character gameplay. Stay Signed In is not enabled. |
...
| `SceneRouter` | Godot scene tree | Transitions among boot, login, register, verify, server-unavailable, account-disabled, recovery, change-password/email, account-delete/deleted, character, and world |
