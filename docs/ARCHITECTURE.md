...
- The match owns live positions, collision, combatants, ground loot, in-memory cooldowns, live quest logs, live inventories, and live equipment loaded from storage. This phase simulates player movement, NPC interaction, quest acceptance, shared slime AI, combat, slime-gel drops, pickup, server-owned inventory, main-hand equip, and derived attack. Quest turn-in and wallet grants are not in this phase.
...
| `NetworkService` | Nakama Godot SDK 3.4.0 | Device auth, in-memory session cache, refresh, reauth, realtime socket, logout, `character_bootstrap`, `find_or_create_starter_zone`, match join/leave, `INPUT`, `INTERACT`, `ATTACK`, `PICKUP`, `EQUIP`, `QUEST_ACCEPT`, `RESYNC_REQUEST`, starter-zone room chat join/leave/send. |
