...
| `quest_offer` / `quest_turn_in` | `quest.ts` | `questIds` |
...
| `respec` | `canonical_respec.ts` | service gate; current legacy overlay is a conflict |

Service `minLevel`, `classRequirements`, `requireParty`, and required quest status are server-authorized gates. Prices belong to vendor item stock and sell multipliers, not NPC dialogue or client requests. Dialogue file paths remain client-local mappings keyed by `dialogueId`; no asset path crosses protocol or storage.

## Current NPC catalog

| IDs | Classification | Notes |
| --- | --- | --- |
| `npc.elder`, `npc.proof_giver`, `npc.cert_quartermaster` | Correct content references | Production starter-zone definitions. Elder carries the accepted slice quest bindings. |
| `npc.test_vendor`, `npc.test_innkeeper`, `npc.test_herald`, `npc.test_cave_portal`, `npc.test_cave_exit` | Test/cert fixture content | Existing generic service coverage and cert journey fixtures; several are currently placed in the starter/cave templates. |
...
| `npc.elder` in `npc.elder.dialogue`, `ContentCatalog.REQUIRED_IDS`, and slice/cert driver | Temporary vertical-slice assumption / test fixture | Replace fixed required-ID and dialogue-action assumptions through a named content/presentation migration. |
| `npc.proof_giver` / `npc.cert_quartermaster` dialogue scripts with literal quest/NPC IDs | Temporary cert/content assumptions | Move choice bindings to validated content/action metadata when a named dialogue-content phase permits it. |
...
| `RESPEC_TRAINER_NPC_IDS = ["npc.test_innkeeper", "npc.lab_trainer"]` in `npc.ts` and `canonical_respec.ts` | Architectural hard-coding requiring migration | Remove the ID overlay after the generic `respec` service is authoritatively represented in content. |
