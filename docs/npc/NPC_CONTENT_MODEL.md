...
| --- | --- | --- |
| `npc.elder`, `npc.proof_giver`, `npc.cert_quartermaster` | Correct content references | Production starter-zone definitions on `route.stationary`. Elder carries the accepted slice quest bindings (`quest_offer` + `quest_turn_in`). Dialogue entry nodes follow offered quest dialogue state. |
| `npc.test_vendor`, `npc.test_innkeeper`, `npc.test_herald`, `npc.test_cave_portal`, `npc.test_cave_exit` | Test/cert fixture content | Generic service coverage and cert journey fixtures. `npc.test_herald` authors one `offer_and_turn_in` bind covering the test quests. `npc.test_innkeeper` authors `respec` and is migrated onto the generic NPC definition in NPC-02 (not deferred). |
