# Current item-system conflicts (ITEM-02)

ITEM-02 closed bag capacity, equipment-out-of-bag, stack caps, production trade/drop, and gel/proof tradeability. Remaining rows stay OPEN until a later numbered ITEM phase names them.
...
| --- | --- | --- | --- | --- |
| ITEM-C01 | CLOSED | Live bag is 30 slots, indices 0–29; HUD “N / 30 stacks” | 6×5 grid remains later | ITEM-02 closed capacity. Grid UI is [ITEM-C15](ITEM_CURRENT_CONFLICTS.md) |
...
| ITEM-C10 | OPEN | `DESTROY_ITEM` deletes; no player ground spawn | Public 5 min ground items; server placement; full-stack pickup | Later ground-drop phase |
...
| ITEM-C13 | OPEN | Container `revision` is live on bag/equipment/overflow; client mutations still use `requestId` only | Monotonic revision + `expected_revision` | Later transaction phase |
| ITEM-C14 | CLOSED | Schema `itemMaxStack` 1–99; equippable forced 1; no production item exceeds 99 | Non-gear 1–99, gear 1 | ITEM-02 |
| ITEM-C15 | OPEN | `CtrlInventory` list; split-half button; no item tooltip; `request_move` unused by HUD | 6×5; tooltips; drag-drop; split/merge/swap | Later UI phase |
...
ITEM-01 reran `trade.test.ts` as part of 856 passing server tests. No retargeting. ITEM-02 reran trade after making gel/proof tradeable; non-tradeable coverage uses a cloned catalog flag.
