# Item protocol catalog (ITEM-01)

Live opcodes from [PROTOCOL_CATALOG.md](../PROTOCOL_CATALOG.md) and `server/src/domain/protocol.ts`. Foundation audit at ITEM-01 baseline: **40** client opcodes, **15** server opcodes, **29** RPCs. ITEM-01 assigns **no** new opcodes.

Client item requests today carry `requestId`, never `expected_revision`. Trade accept carries `revision` (trade record, not bag).
...
| 5 | `EQUIP` | `instanceId?`, `slot`, `requestId` | Ownership, tags, class, level, locks | Successful replay | `unowned`, `not_equippable`, `invalid_slot`, `item_locked`, `class_restricted`, `level_restricted`, `unique_restricted`, `player_dead` |
...
| 105 | `INVENTORY_STATE` | `capacity`, `items[]` (includes `instanceId`, `slotIndex`, lock fields) |
| 109 | `EQUIPMENT_STATE` | `slots`, derived |
