...
Prompt 18 automated suites plus the Prompt 19 freeze audit, Prompt 21 account/character coverage, Prompt 22 progression coverage, Prompt 23 economy coverage, Prompt 24 ability coverage, Prompt 25 combat-pipeline coverage, Prompt 26 enemy/spawn/AI/loot/boss coverage, Prompt 27 NPC/quest/vendor/inn coverage, Prompt 28 party/chat/group-credit/loot coverage, Prompt 29 public-world/cave/transfer/reconnect coverage, Prompt 30 nearby trade coverage, Prompt 31 UI/settings/asset-contract coverage, Prompt 32 content-CLI / systems-lab / GM coverage, Prompt 33 environment / handshake / maintenance / backup-restore coverage, Prompt 34 security / fuzz / rate-limit / capacity / soak / five-client certification, Prompt 35 existing-save / content-only / asset-manifest / five-client resume certification, ACCT-09 account lifecycle security / failure / distribution certification, PROG-01 canonical progression design audit, PROG-02 canonical shared content schemas, PROG-03 four-class creation plus progression-state migration, PROG-04 canonical statistics, derived values, and resource engine, PROG-05 XP curve, automatic growth, milestones, and auto-assignment, PROG-06 manual stat allocation and trainer respec, PROG-07 branch choice, talent trees, ability ownership, and hotbar rules, PROG-08 generic combat mechanics required by all four classes, PROG-09 Warrior / Bulwark / Berserker, PROG-10 Marksman / Sniper / Skirmisher, PROG-11 Mage / Fire / Frost, PROG-12 Mystic / Charms / Curses, PROG-13 character progression and skill-tree UI, PROG-14 progression persistence, lifecycle, leftover migration, and developer tools, PROG-15 deterministic balance simulator and final progression certification, NPC-07 lifecycle/security/content-only NPC platform certification, ITEM-01 item-system contract, ITEM-02 canonical item model / thirty-slot bag / equipment migration, ITEM-03 authoritative container / capacity / lock / transaction core, and ITEM-04 thirty-slot bag UI. Do not weaken these tests.
...
| `npc_vendor.test.ts` | session-gated buy, stock, price spoof, quantity, idempotency | |
| `npc_quest.test.ts` | session-gated accept/turn-in, markers, bind aliases | |
| `npc_security.test.ts` | forged NPC/session, foreign/expired session, range/dead/link-dead/transfer, option/service/quest/merchant injection, spam, payload, join mismatch | |
| `npc_lifecycle.test.ts` | login, resync, switch, disconnect, link-dead, safe leave, restart, transfer, restore, export, deletion, no pose persist | |
| `npc_platform.test.ts` | content-only proof NPCs and two-client journey | |
...
| `character_lifecycle.test.ts` | create warrior/marksman/mage/mystic, invalid class, client stat/level injection, five slots, sixth rejected, invalid name, case-insensitive and concurrent `name_taken`, duplicate create, canonical L1 persist, leftover migrate, export snapshot, foreign/soft-deleted select, ticket expiry/replay, delete/lease/restore/purge (progression removed, restore exact, recreate isolated), Prompt 18 migrate, starter init once | |
| `canonical_progression.test.ts` | L1 arrays, caster/physical mana, production create skips starters, v1 leftover reset, missing blob, hotbar mapping, storage round-trip | |
| `progression_lifecycle.test.ts` | leftover classify/reset, vanguard keep-Foundation, partial/future/interrupted migrate, four-class party XP, forged/out-of-range credit, simultaneous XP, link-dead cast/DoT, reconnect cooldowns, terminate persist, cave/select persist, GM tools, validation | |
| `progression_dps_audit.test.ts` | §6.3 L10 sheet, §12 DPS/HPS ±5%, metronome, greed, T3, DoT no-crit, haste ≠ cooldown | |
| `progression_enemy_baselines.test.ts` | §13 mob/elite baselines L1–10; slime HP remains 20 | |
| `progression_gcd_audit.test.ts` | production GCD search across content, domain, and client scripts | |
| `progression_reference_builds.test.ts` | sixteen reference builds, 27 free points, greed vs vitality | |
| `progression_cert_journey.test.ts` | GM four-class journey, trainer respec, reconnect, export, purge, email reuse | |
| `progression_party_cert.test.ts` | Warrior/Mage/Marksman/Mystic party combat, PvP off, party XP, disconnect | |
| `progression_repository_audit.test.ts` | four-class roster, no client progression writes, no class-id combat branches | |
...
| `mystic_progression.test.ts` | Mystic ATTACK, Fateweave polarity, Charms and Curses talents, one-shot shield terminal heal, Overflow/Vampiric recursion guards, Contagion hop, Malediction | |
| `mystic_balance.test.ts` | Charms DPS/HPS and Curses DPS ±5% | |
| `progression_metronome.test.ts` | Mage Arcane Bolt floor and documented Mystic Fateweave miss | |
...
| `existing_save_cert.test.ts` | Prompt 18, p20, p21, and current fixtures keep quest, gear, gold, pose; second migrate does not duplicate; v1 progression migrates to schema 3 without resetting class/level/XP | |
...
| `account_export.test.ts` | Support Recovery ID format; secret and foreign-data filters; canonical progression fields and `progressionExport` snapshot | |
...
| `npc_avatar_test.gd` | placeholder square, interaction area, no physics body | |
| `npc_platform_content_test.gd` | NPC-07 content-only proof IDs in client catalog | |
...
| `merchant_window_test.gd` | catalog, quantity, gold, Buy, Back; no client price | |
| `cave_service_test.gd` | enter/exit opcodes, transfer metadata, overlay copy | |
| `inventory_service_test.gd` / `bag_ui_test.gd` / `equipment_service_test.gd` / `wallet_service_test.gd` / `progression_service_test.gd` | mirrors; 6×5 bag; tooltips; drag move/swap/merge/split; overflow recover opcode; Inventory Recovery panel; unlock buttons survive HUD refresh | VS-T5, T8 |
