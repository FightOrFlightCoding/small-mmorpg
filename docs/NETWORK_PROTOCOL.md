...
- Limits: `INPUT` **20**, `ATTACK`/`USE_ABILITY`/`CANCEL_CAST`/`SET_TARGET` **8**, `INTERACT` **8**, `PICKUP` **8**, `EQUIP`/`DESTROY_ITEM`/`SPLIT_STACK`/`MOVE_ITEM` **8**, `QUEST_ACCEPT`+`QUEST_TURN_IN`+`VENDOR_BUY`+`VENDOR_SELL`+`INN_REST`+`CAVE_ENTER`+`CAVE_EXIT` **8**, `ALLOCATE_ATTRIBUTES`/`ASSIGN_HOTBAR`/`UNLOCK_ABILITY`/`RELEASE_RESPAWN` **8**, `RESYNC_REQUEST` **2**. Extra requests are `rate_limited` (`SYSTEM_MESSAGE`), are logged, and do not apply. At most **24** match messages are parsed per player per tick.
...
| 103 | `ACTION_RESULT` | `{ protocolVersion, ok, code, requestId?, ticketId?, destinationMatchId?, destinationInstanceId?, originMatchId?, zoneId?, instanceType? }` |
...
- party member lists, credit recipients, loot recipients, destination match ids, instance ids, or transfer tickets the client invented
...
- wrong content hash
- missing/malformed `requestId` on `INTERACT`, `ATTACK`, `USE_ABILITY`, `CANCEL_CAST`, `ASSIGN_HOTBAR`, `UNLOCK_ABILITY`, `EQUIP`, `DESTROY_ITEM`, `SPLIT_STACK`, `MOVE_ITEM`, `ALLOCATE_ATTRIBUTES`, `VENDOR_BUY`, `VENDOR_SELL`, `INN_REST`, `CAVE_ENTER`, `CAVE_EXIT`, and reward opcodes
