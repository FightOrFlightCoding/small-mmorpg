...
- Join metadata carries `protocolVersion`, `contentHash`, `clientVersion`, and either `selectionTicket` or `transferTicket` as strings (Nakama metadata is string-valued). `characterId` is rejected as `stat_injection`.
...
- Limits: `INPUT` **20**, `ATTACK`/`USE_ABILITY`/`CANCEL_CAST`/`SET_TARGET` **8**, `INTERACT`/`DIALOGUE_CHOOSE`/`INTERACTION_CLOSE` **8**, `PICKUP`/`OPEN_CORPSE`/`CLOSE_CORPSE`/`CLAIM_CORPSE_ITEM`/`CLAIM_CORPSE_GOLD`/`LOOT_ALL_CORPSE`/`SUBMIT_LOOT_ROLL`/`PICKUP_GROUND_ITEM` **8**, `DESTROY_ITEM`/`SPLIT_STACK`/`MOVE_ITEM`/`RECOVER_OVERFLOW_ITEM`/`DROP_ITEM` **8**, `EQUIP` **8**, `QUEST_ACCEPT`+`QUEST_TURN_IN` **8**, `VENDOR_BUY`+`VENDOR_SELL`+`INN_REST` **8**, `CAVE_ENTER`+`CAVE_EXIT` **8**, trade opcodes **8**, `ALLOCATE_ATTRIBUTES`/`ALLOCATE_ATTRIBUTES_BATCH`/`TRAINER_RESPEC`/`PURCHASE_TALENT`/`ASSIGN_HOTBAR`/`UNLOCK_ABILITY`/`RELEASE_RESPAWN`/`RETURN_TO_CHARACTER_SELECT`/`SELECT_BRANCH`/`SET_AUTO_ASSIGN`/`AUTO_ASSIGN_UNSPENT_POINTS` **8**, `RESYNC_REQUEST` **2**. Extra requests are `rate_limited` (`SYSTEM_MESSAGE`), are logged, and do not apply. At most **24** match messages are parsed per player per tick. Session limits: auth **5 / 10 s**, chat **4 / 2 s**, mutating party RPCs **8 / 2 s** (`party_get_state` exempt).
...
| 5 | `EQUIP` | `{ protocolVersion, instanceId?, slot, requestId }` | Equip or unequip a content-defined slot (`main_hand`, `off_hand`, `head`, `chest`, `legs`, `feet` in the current catalog). Equip moves the instance out of the bag into `equipment.items`. Omit `instanceId` to unequip into a free bag slot (`inventory_full` and no mutation when the bag cannot hold it). `requestId` required. Client `attack` / `attackBonus` are rejected. |
| 6 | `QUEST_ACCEPT` | `{ protocolVersion, interactionSessionId, npcInstanceId, questId, requestId }` | Reward opcode. Requires a live interaction session. Validates that the NPC offers the quest, plus quest ID, range, prerequisites, level, and class; creates accepted state once; persists; returns `ACTION_RESULT`, `QUEST_STATE` (including character-specific `npcQuestMarkers`), and accepted dialogue on `INTERACTION_RESULT`. |
| 7 | `QUEST_TURN_IN` | `{ protocolVersion, interactionSessionId, npcInstanceId, questId, requestId }` | Reward opcode. Requires a live interaction session. Validates that the NPC accepts turn-in, range, accepted quest, server-side objectives, and required items. Client `gold` / `questComplete` / `resultingGold` / `resultingBalance` / `npcId` are rejected. |
| 8 | `RESYNC_REQUEST` | `{ protocolVersion }` | Replies with a fresh `FULL_STATE`. |
| 9 | `ALLOCATE_ATTRIBUTES` | `{ protocolVersion, attributeId?, statId?, amount, requestId }` | Spend unspent free points. `statId` aliases `attributeId`. `amount` is a finite integer. Client `xp` / `level` / `currentXp` are rejected. |
...
| 19 | `VENDOR_BUY` | `{ protocolVersion, interactionSessionId, vendorId, stockEntryId, quantity?, preferredSlot?, requestId, expectedRevision? }` | Reward opcode. Requires a live interaction session. Server stock and prices. Client `price` / `gold` / `resultingBalance` rejected. Quantity omitted means 1. |
...
| 41 | `RECOVER_OVERFLOW_ITEM` | `{ protocolVersion, instanceId, toSlotIndex?, requestId }` | Move one MigrationOverflow stack into a free bag slot. Not extra storage. `requestId` required. |
| 42 | `OPEN_CORPSE` | `{ protocolVersion, corpseId, requestId }` | Open a corpse loot window. Range and eligibility stay server-authoritative. |
| 43 | `CLOSE_CORPSE` | `{ protocolVersion, corpseId, requestId }` | Close the caller's corpse view. |
| 44 | `CLAIM_CORPSE_ITEM` | `{ protocolVersion, corpseId, entryId, toSlotIndex?, requestId, expectedRevision? }` | Claim one corpse item. Optional preferred bag slot. No recipients. |
| 45 | `CLAIM_CORPSE_GOLD` | `{ protocolVersion, corpseId, requestId }` | Claim corpse gold. Private split or public remainder. No amount. |
| 46 | `LOOT_ALL_CORPSE` | `{ protocolVersion, corpseId, requestId, expectedRevision? }` | Gold then items. Detailed `lootAll[]` on `ACTION_RESULT`. |
| 47 | `SUBMIT_LOOT_ROLL` | `{ protocolVersion, rollId, choice, requestId }` | Need/Greed/Pass. Client never sends a roll number. |
| 48 | `DROP_ITEM` | `{ protocolVersion, instanceId, quantity?, hintDx?, hintDy?, requestId, expectedRevision? }` | Public ground drop. Client never sends a trusted pose. |
| 49 | `PICKUP_GROUND_ITEM` | `{ protocolVersion, groundEntityId, requestId, expectedRevision? }` | All-or-nothing public ground pickup. |
...
| Opcode | Name | Body |
| --- | --- | --- |
| 101 | `FULL_STATE` | `{ protocolVersion, contentHash, tick, zoneId, selfId, players, npcs, enemies, loot, corpses, groundItems, quests, npcQuestMarkers, inventory, equipment, derived, wallet, progression, abilities, party?, instance? }` |
| 102 | `SNAPSHOT` | `{ protocolVersion, contentHash, tick, zoneId, players, enemies, loot, corpses, groundItems }` |
| 103 | `ACTION_RESULT` | `{ protocolVersion, ok, code, requestId?, ticketId?, destinationMatchId?, destinationInstanceId?, originMatchId?, zoneId?, instanceType?, tradeId?, lootAll? }` |
| 104 | `COMBAT_EVENT` | `{ protocolVersion, tick, events }` |
| 105 | `INVENTORY_STATE` | `{ protocolVersion, contentHash, requestId?, capacity, items, revision?, overflow? }` |
| 106 | `QUEST_STATE` | `{ protocolVersion, contentHash, requestId?, quests, npcQuestMarkers? }` |
...
| 109 | `EQUIPMENT_STATE` | `{ protocolVersion, contentHash, requestId?, slots, items?, revision?, derived }` |
...
| 116 | `CORPSE_STATE` | `{ protocolVersion, contentHash, requestId?, corpse }` |
| 117 | `CORPSE_REMOVED` | `{ protocolVersion, contentHash, corpseId, reason }` |
| 118 | `LOOT_ROLL_STATE` | `{ protocolVersion, contentHash, requestId?, roll }` |
| 119 | `GROUND_ITEM_REMOVED` | `{ protocolVersion, contentHash, groundEntityId, reason }` |

`FULL_STATE` is sent to the joining presence after character, quest, inventory, equipment, wallet, and progression load, and again on `RESYNC_REQUEST`. Occupied matches broadcast `SNAPSHOT` every tick (10 Hz) with player poses, enemies, ground loot, corpses, and player-created ground items. Each player record includes `x`, `y`, `health`, `maxHealth`, `alive`, `lastProcessedSeq`, `resources`, `effects`, `activeCast`, `inCombat`, `hostileTargetId`, `friendlyTargetId`, `deadUntilTick`, `stunned`, and `rooted`. Each enemy record includes `id`, `enemyId`, `x`, `y`, `health`, `maxHealth`, `alive`, `state`, `phaseId`, `aiProfileId`, and `effects`. `quests`, `npcQuestMarkers`, `inventory`, `equipment`, `derived`, `wallet`, `progression`, and `abilities` on `FULL_STATE` are the recipient's records only. `npcQuestMarkers` is `{ npcId, marker }` with glyphs `!` available, `?` ready to turn in, `·` active incomplete; `SNAPSHOT` never includes it. Optional `party` is the recipient's server party view (ids, leader, members, revision, connection state). Optional `instance` is `{ type, instanceId, zoneTemplateId, completionState, bossAlive, ownerPartyId?, ownerCharacterId? }`. `abilities` is `{ unlockedAbilityIds, hotbar, abilityRanks, resources, cooldowns, globalCooldownRemaining, activeCast, effects }`. Public loot is `{ id, itemId, quantity, x, y, expiresAtTick }` and does not include item instance IDs. Public player ground items are `{ id, groundEntityId, itemId, quantity, x, y, expiresAtTick, rarity, state }` and omit instance ids and `createdByCharacterId`. A client that receives no snapshot or full state for **2 seconds** freezes remote interpolation and shows a degraded-connection state (`snapshot_timeout`). Local prediction still reconciles when snapshots resume. Dialogue opens only after `INTERACTION_RESULT` `ok`. `COMBAT_EVENT.events` entries are `{ type, sourceId, sourceKind, targetId, targetKind, damage?, healing?, remainingHealth?, x?, y?, respawnDelaySec?, interruptReason?, effectId?, abilityId?, resourceId?, resourceDelta?, message? }` with `type` `hit`, `heal`, `death`, `respawn`, `interrupt`, `effect_applied`, `effect_tick`, `resource`, `threat`, `credit`, or `message`. Damage and healing numbers are presentation only.
...
- wrong content hash
- missing/malformed `requestId` on `INTERACT`, `DIALOGUE_CHOOSE`, `INTERACTION_CLOSE`, `ATTACK`, `USE_ABILITY`, `CANCEL_CAST`, `ASSIGN_HOTBAR`, `UNLOCK_ABILITY`, `PURCHASE_TALENT`, `EQUIP`, `DESTROY_ITEM`, `SPLIT_STACK`, `MOVE_ITEM`, `RECOVER_OVERFLOW_ITEM`, `ALLOCATE_ATTRIBUTES`, `ALLOCATE_ATTRIBUTES_BATCH`, `VENDOR_BUY`, `VENDOR_SELL`, `INN_REST`, `CAVE_ENTER`, `CAVE_EXIT`, trade opcodes, corpse opcodes, and reward opcodes
...
- Tick rate: **10 Hz**
- Join loads the character, quest, inventory, equipment, overflow, and wallet once. Join runs `migrateItemContainers` (bag 30, equipment extract, overflow leftover) and persists changed records. The tick loop does not read storage. Quest acceptance and objective progress write `collection` `player`, key `quests`, `permissionWrite: 0`. Successful pickup writes `collection` `player`, key `inventory`, `permissionWrite: 0`. Successful equip or unequip writes inventory and equipment. Successful overflow recover writes inventory and overflow (or deletes empty overflow). Successful turn-in, vendor buy/sell, and inn rest use `nk.multiUpdate` for inventory, quests, and wallet gold together when those records change. Public-world position checkpoints write `collection` `player`, key `character` every 5 seconds if the pose changed, on leave, and on match terminate, and they persist inn bind when `bindX`/`bindY` are set. Cave matches skip periodic character-position checkpoints. Ground loot is match-only and is not persisted. Health is not written.
- Join metadata must include matching `protocolVersion`, `contentHash`, and `clientVersion`, plus a live `selectionTicket` on a new non-transfer join or a live `transferTicket` for a transfer join
- A second socket while a live lease exists is `account_busy` or `link_dead`. Same-session resume of an **ONLINE** presence is allowed. A link-dead entity is not rebound to a new socket. After unexpected disconnect the entity stays in `SNAPSHOT`/`FULL_STATE` for **10 server seconds after detection** (`linkDead: true`), remains targetable, and still takes PvE damage. Movement and player commands are rejected. Trade cancels immediately. After despawn, join loads the checkpointed position and `joinHealth`. Conflicting presence in another running match is `already_elsewhere` unless a valid transfer is in flight. The same account cannot occupy two character entities. Detection is Nakama presence loss, not window-close time: defaults `ping_period_ms` 15000 and `pong_wait_ms` 25000, so a frozen client may wait up to 25 s before the 10 s hold starts.
- Logout and Character Select wait for opcode **32** acknowledgement, then leave the match. Failed safe leave stays in the world with a visible reason. Quit Anyway / process kill use the link-dead path.
...
- If the realtime socket drops, the client restores the session/socket with bounded exponential backoff (0.5s doubling to 8s, 8 attempts) and does **not** rejoin the match during the ten-second link-dead policy. It returns to Character Select (`Character remains in world`) and uses `playAvailableAt` for the countdown. Cancel/Log out still returns to login. A new Play after the lease clears requires a new selection ticket.
