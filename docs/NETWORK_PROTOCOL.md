...
- Per-player action windows live on match state (`actionRates`) and reset every **10 ticks** (1 second at 10 Hz). Limits: `INPUT` **20**, `ATTACK` **8**, `INTERACT` **8**, `PICKUP` **8**, `EQUIP`/`DESTROY_ITEM`/`SPLIT_STACK`/`MOVE_ITEM` **8**, `QUEST_ACCEPT`+`QUEST_TURN_IN` **8**, `ALLOCATE_ATTRIBUTES` **8**, `RESYNC_REQUEST` **2**. Extra requests are `rate_limited` (`SYSTEM_MESSAGE`), are logged, and do not apply. At most **24** match messages are parsed per player per tick.
...
| 3 | `ATTACK` | `{ protocolVersion, targetId, requestId }` | Correlation `requestId` required. Server checks live health, enemy existence, range vs `player.base.attackRange`, and cooldown vs `player.base.attackCooldown`. Damage is the server's canonical derived attack (stat pipeline when the player has a class and progression; otherwise `player.base.attack` plus equipped main-hand `attackBonus`). Client `damage` / `attack` / `xp` are rejected. Returns `ACTION_RESULT` plus `COMBAT_EVENT` on a hit. |
...
| Opcode | Name | Body |
| --- | --- | --- |
| 101 | `FULL_STATE` | `{ protocolVersion, contentHash, tick, zoneId, selfId, players, npcs, enemies, loot, quests, inventory, equipment, derived, wallet, progression }` |
...
- wrong content hash
- missing/malformed `requestId` on `INTERACT`, `ATTACK`, `EQUIP`, `DESTROY_ITEM`, `SPLIT_STACK`, `MOVE_ITEM`, `ALLOCATE_ATTRIBUTES`, and reward opcodes
