...
After a visual, tileset, or catalog change is on `origin/main`, the Windows Godot checkout tests it with `powershell -File scripts/local-play.ps1 -Branch main` (close the editor first). Remote agents cannot copy files into `C:\Users\Eszter\small-mmorpg`.
