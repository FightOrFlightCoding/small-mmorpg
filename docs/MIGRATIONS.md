...
| Flag | Meaning |
| --- | --- |
| `--fixture path` | JSON snapshot (character, inventory, equipment, optional overflow, quests, optional walletRef, gold) |
...
PROG-14 does not raise `SAVE_SCHEMA_VERSION`. Character progression blobs gain `progressionSchemaVersion` **3** after leftover Foundation-field cleanup. Production leftover `allocatedAttributes`, live `hotbar`, Foundation unspent counters, and `test.ability.*` unlocks are reset with notice `leftover_foundation_reset`. `test.*` characters keep those fields. Unsupported future versions are rejected and not rewritten. Soft-delete restore does not regrant. Account deletion and character purge remove the blob.
