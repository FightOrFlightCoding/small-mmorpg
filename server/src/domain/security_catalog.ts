...
export const SECURITY_CONTROLS: SecurityControl[] = [
  ...
  row("attribute_overspending", "progression", "Spend more points than unspent", "allocateAttributes checks amount and pool", "allocate 8/10 ticks", MATCH, "requestId replay", "insufficient_points / invalid_amount", ["progression.test.ts", "progression_respec.test.ts"]),
  ...
  row("respec_gold_replay", "progression", "Replay trainer respec to double-charge gold", "TX_REASON_RESPEC requestId plus respecByRequestId", "allocate 8/10 ticks", MATCH, "requestId", "replay ok; gold deducted once", ["progression_respec.test.ts"]),
  row("talent_point_overspending", "progression", "Spend class points in a branch tree or overspend ranks", "purchaseTalent checks treeKind, pools, tiers, and prerequisites", "allocate 8/10 ticks", MATCH, "requestId replay", "insufficient_points / invalid_tree / prerequisite_missing", ["progression_talent_trees.test.ts"]),
  row("production_unlock_any", "progression", "UNLOCK_ABILITY on a production class", "Production classes reject UNLOCK_ABILITY", "allocate 8/10 ticks", MATCH, "requestId", "unsupported_class", ["progression_hotbar_ceiling.test.ts"]),
  row("client_crit_roll", "combat", "Client submits crit or random rolls", "OUTCOME_KEYS reject crit/critRoll/random/roll; server CombatRandom only", "attack 8/10 ticks", MATCH, "requestId", "stat_injection:crit", ["progression_combat_mechanics.test.ts", "combat_rng.test.ts"]),
...
export const REQUIRED_SECURITY_IDS: string[] = [
  ...
  "talent_point_overspending",
  "production_unlock_any",
  "client_crit_roll",
  "position_spoofing",
  "speed_hacking",
  "target_spoofing",
